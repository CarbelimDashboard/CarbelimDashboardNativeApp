...
export default {
  data() {
    return {
      ...
      selectedParameter: '',
      intervalId: null,
      hasError: false, // Flag to track if an error alert has been shown
    };
  },
  computed: {
    isDeviceValid() {
  ...
  methods: {
    async getGraphData() {
      if (this.isDeviceValid) {
        try {
          ...
          this.hasError = false; // Reset error flag on successful data fetch
        } catch (error) {
          this.handleError();
        }
      }
    ...
    handleError() {
      if (!this.hasError) { // Check if error alert has not been shown
        this.hasError = true; // Set flag to prevent duplicate alerts
